---
name: level-up
description: Use weekly to find and ship one new automation. Walks the Operator Loop interview — Mindset (find the candidate) → Method (scope one) → Machine (build it). Trigger on "let's level up", "what should I automate next", "find me leverage this week", or as a Friday ritual. One run = one shipped artifact.
---

## What this skill does

Walks the user through the Operator Loop each week to surface and ship one new automation. **One interview = one artifact.** It also installs the Operator Loop framework into the user's head over time — after 4-6 runs, the user starts spotting opportunities mid-week without prompting because the questions have become internal defaults.
...
- `references/operator-loop.md` — the framework (used to quote principles back)
...
- **Dry run on a filled-in setup** with no prompt. Expected: skill surfaces 2-3 candidates pulled from the user's recent activity, priorities, and top_pain. Generic output ("you should build a brief") = fail.
